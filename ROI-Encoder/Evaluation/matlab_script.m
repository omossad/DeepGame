...
%comp = {'Base-x265','ObjDet','DeepGame', 'Cave'};
comp = {'Base-x265','Lambda-ROI'};
y_lim_psnr_std=[0 15];
...
base_path_ = [strcat({'C:\\Users\\omossad\\Desktop\\dataset\\encoding\\ga1\\'})];
tic
...
for ss=1:sequences
        ...
        for k=1:numel(x_tick_const)
			...
			y_lim_psnr=[min(PSNR_base, PSNR_lambda_r)-1];
      f = figure('visible','off');
        plot(actual_base_rate/scale,PSNR_base,'-.b^');
        display(actual_base_rate/scale)
        display(PSNR_base)
        hold on
        plot(actual_lambda_r_rate/scale,PSNR_lambda_r,'-ro');
        display(actual_lambda_r_rate/scale)
        display(PSNR_lambda_r)
        hold on

        set(gca,'xticklabel',x_axis,'xtick',x_tick)
        xlabel(char(strcat('Bitrate ',bw)));
        ylabel('PSNR (dB)');
				%legend('Base','ObjDet','DG','CAVE');
legend('Base','DeepGame');
        xlim(x_lim_rate)
        saveFig(f,char(strcat(base_path,'temp\\PSNR',sequence)));

      y_lim_psnr_std=[0 min(std_base,std_lambda_r)+1];
      f = figure('visible','off');
...
legend('Base','DeepGame');
        ...
        min_ssim=min(min(SSIM_base),min(SSIM_lambda_r));
...
legend('Base','DeepGame');
        ...
        min_ssim=min(min(base_roi_ssim), min(lambda_r_roi_ssim));
		if(min_ssim<ticks_ssim_5(1))
			min_ssim=ticks_ssim_1;
			fmt='%.1f';
			q=1;
